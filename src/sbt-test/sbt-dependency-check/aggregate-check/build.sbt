      ...
      logFiles match {
        case Some(logfile) =>
          scala.util.Using(scala.io.Source.fromFile(logfile)) { logs =>
            ...
            assert(!lines.contains("jackson-databind-2.9.9.jar"), "'jackson-databind-2.9.9.jar' should be not scanned since it's coming from 'ignore', and it's not aggregated")
          }.get
